// pages/quit-handle/index/index.js
const app = getApp()
Page({
    ...
    wx.login({
      success (res) {
        if (res.code) {
          ...
          let rendata = app.requestfun(data, '/Api/invites/index');    
          rendata.then((v) => {
            if(v.data.userlist&&v.data.userlist.length>=1){
                that.setData({
  ...
  show:function(e){
    ...
    wx.navigateTo({
      url: '/pages/quit-handle/details/details?list=' + list + '&type=' + type + '&search=' + search +'&id=' +id,
    })
